...
public class SearchTree implements INodeList {
    ...
    public void traverse(ListItem item) {
        ...
        // Traverses the binary tree left and right, at the same time, until it reaches roots without children:
        if (root != null) {

            // Goes to the left root (previous) recursively. Finishes when the root has no children (null):
            traverse(root.previous());
            System.out.println(root.value);

            // Goes to the right root (next) recursively. Finishes when the root has no children (null):
            traverse(root.next());
            System.out.println(root.value);
        }
    }
